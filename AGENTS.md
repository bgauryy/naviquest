...
Yarn 4. `@naviquest/core` lives in `packages/naviquest/` — a browser WebMCP SDK: six token-budgeted tools on `document.modelContext`. Agents ask the page; they do not ingest it. Two callers, one implementation: a site embeds `createNaviquest` and `register()`, or an automation host injects the same SDK (the install-script helper in `eval/eval.ts`, `docs/TESTING.md`) into **any** origin — no page markup required. `orientation` / `exclude` are optional first-party overlays. Layout: `packages/naviquest/` SDK · `apps/demo/` CityDesk (not a package; `/`, `/parking.html`, `/libraries.html`, `/notices.html` on `:5310`) · `eval/` sensors · `docs/` + `ARCHITECTURE.md`.
