...
// The runner's own imports must be readable under the permission model, and
// each one has to be allowed by both its path and its realpath.
const RUNNER_IMPORTS = ['artifacts.mjs', 'mandatory-stealth.mjs', 'undercover.mjs', 'cdp-client.mjs', 'reader-prime.mjs']
  .map((name) => resolve(__dir, name));
...
const OUTPUT_DIR = join(OUTPUT_BASE, 'tmp', 'chrome', timestamp);
mkdirSync(OUTPUT_DIR, { recursive: true, mode: 0o700 });
const SESSION_META_DIR = join(OUTPUT_BASE, 'tmp', 'chrome', 'session-meta', `port-${PORT}`);
mkdirSync(SESSION_META_DIR, { recursive: true, mode: 0o700 });

const safePath = (p) => { try { return realpathSync(p); } catch { return p; } };

const TMPDIR_RAW  = OUTPUT_BASE;
...
const HELPERS = ['undercover.mjs', 'mandatory-stealth.mjs', 'dom-actionability.mjs'];
...
if (VERBOSE) {
  ...
  console.error(`[CDP_SANDBOX]  FS read:       .naviquest output tree + runner`);
