...
`eval/eval.ts` launches its own Playwright Chromium and bundles the SDK itself —
no server, no skill, no setup. Offline lanes:
...
Live lanes drive real pages (MDN by default), so they can fail on an outage —
that is why they never run by default. Artifacts land in `eval/out/`
