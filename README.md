...
origin offers agents — and every one of them returns a budgeted result.
...
console.log(hit.results[0]?.text);
...
> ⚗️ **This is a POC, and the numbers are a POC‑scale sample** — 20 findings,
...
The host owns navigation, clicks, typing, and screenshots — Naviquest returns the semantic target, live state, box, and navigation provenance for it to act on. What page JS cannot do (privileged input, cross‑origin frames, screenshots) is reported in `coverage`, never silently omitted.

---

## Requirements

Core SDK: a **WebMCP‑capable browser**. `document.modelContext` is not in stock Chrome — enable `chrome://flags/#enable-webmcp-testing` and relaunch. It runs on any origin; repo tooling needs **Node 22+** and **Google Chrome**.
...
| Path | Contents |
|---|---|
| `packages/naviquest/` | `@naviquest/core`, the publishable package |
